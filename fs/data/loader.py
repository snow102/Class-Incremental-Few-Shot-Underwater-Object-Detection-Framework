...
from .vocanno import VocAnnotation, VocInstance, DatasetMetaInfo, VocDataset
...
class VocAnnoLoader(VocDataset):
    BASE_CATEGORIES = None
    DATA_DIR = None  #
    ImageDirName = "JPEGImages"
    AnnoDirName = "Annotations"

    def __init__(self, dsMeta: DatasetMetaInfo) -> None:
        super().__init__()
        self._meta = dsMeta
        self.name = dsMeta.full_name
        self.use_more_base = 'ploidy' in self.name
        self.is_shots = "shot" in self.name
        self.dirname = dsMeta.dirname
        self.split = dsMeta.type
    ...
    def filter_for_tsne(self, annos: list, classnames: list):
        ...
        if not manaual_select:  # 如果是非手动选择（随机自动选择），
            desired_annos = []
    ...
    def load_instance_from_file_ex(fileids, classnames: "list", dirname: str, 
                                   annoDir="Annotations", imgDir="JPEGImages", design_classnames=None):
        ...
        bsf.c 注意这里如果过滤掉 Base Object, Offset 应该设为 len(BaseClassList)
        """
        assert classnames is not None, "API has changed, please check params passed in"
        if design_classnames is None:
            design_classnames = classnames
        total_annotations = []
        idx = 0
        for fileid in fileids:
            ...
            for obj in tree.findall("object"):
                ...
                if not (cls in design_classnames):
                    continue
    ...
    def save_instance(annotaion: "VocAnnotation", dst_loc: str):
        ...
        width.text = str(annotaion['width'])
        height = ET.SubElement(size, "height");
        height.text = str(annotaion['height'])
        depth = ET.SubElement(size, "depth");
        depth.text = str(annotaion.get('depth', 3))
    ...
    @staticmethod
    def load_from_metafile(dsMeta: DatasetMetaInfo):
        ...
        VocAnnoLoader.set_class_names(classnames)
        fileids = get_imagesets_from_file(dirname, split)
        return VocAnnoLoader.load_instance_from_file_ex(fileids, classnames, dirname)
    ...
    def load_from_meta(self):
        ...
        VocAnnoLoader.set_class_names(dsMeta.classnames)
        fileids = get_imagesets_from_file(dsMeta.dirname, dsMeta.type)
        for fileid in fileids:
            image_dir = osp.join(dsMeta.dirname, "JPEGImages")
            file_loc = osp.join(dsMeta.dirname, "Annotations", f"{fileid}.xml")
            self.parse_voc_annotation(file_loc, image_dir)
