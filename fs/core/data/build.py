...
import logging
logger = logging.getLogger("fsdet.fs.data.build")
...
def print_instances_class_histogram(dataset_dicts, class_names):
    ...
    histogram_e = np.zeros((num_classes,), dtype=np.int)
    for entry in dataset_dicts:
        if any(c.isalpha() for c in entry['image_id']):
            annos = entry["annotations"]
            classes = [x["category_id"] for x in annos if not x.get("iscrowd", 0)]
            histogram_e += np.histogram(classes, bins=hist_bins)[0]
            continue
        annos = entry["annotations"]
        classes = [x["category_id"] for x in annos if not x.get("iscrowd", 0)]
        histogram += np.histogram(classes, bins=hist_bins)[0]
    logger.info(f"Histogram {histogram_e}")
    ...
    log_first_n(
        logging.INFO,
        f"FS Distribution of training instances among all {num_classes} categories:\n"
        + colored(table, "cyan"),
        key="message",
        name="fsdet.data.build"
    )
...
def build_detection_train_loader(cfg, mapper=None):
    ...
    # aspect_ratios = [float(img.get("height", 1.0)) / float(img.get("width", 1.0)) for img in dataset ]
...
def build_detection_test_loader(cfg, dataset_name, mapper=None):
    ...
    batch_sampler = torch.utils.data.sampler.BatchSampler(sampler, cfg.SOLVER.TEST_IMS_PER_BATCH, drop_last=False)
