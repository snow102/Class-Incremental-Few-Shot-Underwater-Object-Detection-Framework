...
from fsdet.evaluation.pascal_voc_evaluation import PascalVOCDetectionEvaluator
from fsdet.config import globalvar as gv
now=datetime.now()
dirname = f"checkpoints/fs/pascal_voc_eval_tfa{now.month:02d}{now.day:02d}_{now.hour:02d}{now.minute:02d}"
os.makedirs(dirname, exist_ok=True)

logger = logging.getLogger("fs.evaluation.evaluator")

class FsDetectionEvaluator(PascalVOCDetectionEvaluator):
    """bsf.c 与 PascalVOCDetectionEvaluator 一致，目前修改了 process 方法 一遍进行额外处理
